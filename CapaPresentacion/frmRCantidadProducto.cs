...
{
    public partial class frmRCantidadProducto : Form
    {
        public frmRCantidadProducto()
        {
            InitializeComponent();
        }

        private void frmRCantidadProducto_Load(object sender, EventArgs e)
        {
            try
            {
                string bandera = frmReporteCantidadProductosVenta.f1.lblBandera.Text;
                string fechaInicio = "";
                string fechaFin = "";
                DateTime fecIn;
                if (bandera == "1")
                {
                    fechaInicio = frmReporteCantidadProductosVenta.f1.dtpFechaInicio.Value.ToString("yyyy-MM-dd" + " 00:00:00");
                    fechaFin = frmReporteCantidadProductosVenta.f1.dtpFechaFin.Value.ToString("yyyy-MM-dd" + " 23:59:59");
                }
                else
                {
                ...
                repdoc.Load(@"D:\Reportes\RVentaCantProducto.rpt");
